...
目标场景是：在一台专用银河麒麟电脑上，通过桌面图标双击启动，完成物资建档、入库、出库、库存、调拨依据与单位追溯、组合查询、Excel 导出以及数据备份/恢复。
...
- 物资品名管理
- 计量单位和存放位置
- 内置常用计量单位（发、支、具、件、套、枚、米、公斤、箱），并支持现场新增
- 入库登记
- 出库登记
- 数量按原值录入（最多两位小数，超出时拒绝而非四舍五入）
- 业务日期只录入年月日
- 来源/领用单位支持输入新增、搜索和选择
- 入库/出库支持多物资一次确认，出库只填写领用单位
- 入库/出库支持 USB 条码枪；扫描单据可调用 Tesseract OCR 生成待核对明细
- 库存不足阻止出库
- 当前库存
- 库存物资分布
- 出入库流水
- 物资备注、入库备注和出库备注可附加单据图片，并可在流水中回看
- 名称 / 时间 / 单位 / 类型 / 调拨依据组合查询
- 当前查询结果 XLSX 导出
- 物资明细导出
- 出入库明细导出
- 库存物资分布导出
...
单据图片以 BLOB 形式随业务记录保存在 SQLite 中，不依赖原图片路径，因此会随即时/年度备份一起备份和恢复。恢复业务数据时会先生成恢复前安全副本，再执行数据库替换和完整性检查。
...
V1 是轻量物资管理系统，不包含采购 ERP、财务、销售、在线支付、SaaS、多级复杂审批或第三方 ERP/WMS 集成。USB 条码枪可直接输入条码；扫描单据 OCR 依赖麒麟系统安装 `tesseract-ocr` 与中文语言包。新增范围必须先更新需求文档。
